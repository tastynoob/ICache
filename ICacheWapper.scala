...
class ICache_tagArray(tag_size: Int, line_nums: Int) extends Module {
    ...
    val sram = Module(new RAM2_without_wmask(tag_size, line_nums))
    when(wc.vld_update) {
...
class ICache_dataArray(data_size: Int, line_nums: Int) extends Module {
    ...
    val mem = Module(new RAM2_without_wmask(data_size, line_nums))
...
class ICacheWay extends Module with ICacheConfig {
    val fetch = IO(new Bundle {
        ...
        val data = Output(UInt(cacheLineWid.W))
    })
    ...
    val refill = IO(new Bundle {
        ...
        val data = Input(UInt(cacheLineWid.W))
    })
    ...
    val data_array = Module(new ICache_dataArray(cacheLineWid, lineNums))

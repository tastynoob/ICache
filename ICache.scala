...
class ICache extends Module with ICacheConfig with L2Config {
    /** core req bus */
    val fetchBus = IO(Flipped(new RTIB1_interface(fetchAddrWid, fetchWid)))
    /** external stroage bus */
    val extBus = IO(new RTIB1_interface(l2addrwid, l2datawid))

    val extReq = RegInit(false.B)
    val extAddr = Reg(UInt(l2addrwid.W))
    extBus.req := extReq
    extBus.addr := extAddr
    extBus.wrcs := false.B
    extBus.wmask := 0.U
    ...
    val ways_datas = Wire(Vec(wayNums, UInt(cacheLineWid.W)))
    ...
    //stage2
    //
    val data_output = Reg(UInt(cacheLineWid.W))
